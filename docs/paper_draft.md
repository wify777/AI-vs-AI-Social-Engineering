...
**The null sharpens the paper rather than weakening it.** A small positive effect for the confirmation mechanism would have been the harder result to interpret: at these sample sizes it would have sat close to the noise floor and invited a debate about whether the defense "helps a little." Two clean nulls against a 13.3pp model effect give an unambiguous ordering of where robustness lives in this threat model — in the choice of model, not in the prompt-level scaffolding wrapped around it.

That reading is reinforced by §4.5, where the *other* prompt-level intervention we examined behaved differently but pointed the same way. A defensive system prompt drove measured ASR to zero for every model at once, erasing the 13.3pp spread that the neutral prompt reveals. So across our two in-context manipulations, one flattened the measurement uniformly and the other did nothing at all; neither produced *differential* robustness. Prompt-level scaffolding in our experiments changes what the benchmark can see, or changes nothing — it does not make a vulnerable model behave like a resistant one.

**What we cannot conclude from this.** It is tempting to read the pattern as evidence that training-time safety is the mechanism that does work, since model identity is what carries the effect. Our design does not support that step. We never manipulated training: the six models differ simultaneously in vendor, pretraining data, architecture, parameter count, and post-training procedure, and nothing in our data isolates which of these produces the spread. "Robustness tracks the model" is what we measured; "robustness comes from RLHF" is a further claim requiring an experiment we did not run. The current design therefore cannot distinguish training-time safety from the other differences between models, and we do not claim it does; §5.6 specifies a controlled within-family comparison that would isolate this factor and clarify the mechanism. We also note that the §4.5 defensive prompt did, in the narrow sense, prevent every tool execution — so a blanket statement that prompt-based defenses fail would overstate our own result. The defensible claim is narrower and still useful: **the two in-context defenses we tested did not differentiate robust models from vulnerable ones, while the choice of model did.**
...
- **Isolate the training-time effect (highest-value causal test).** Testing whether training-time safety (RLHF/DPO) *causes* robustness requires a controlled comparison within a single model family where only post-training differs. Suggested design: take one base checkpoint — Llama-3-8B is a convenient choice, since the base weights and several openly-documented post-trained descendants share it — and run the identical benchmark across (a) the raw base model, (b) the official instruction-tuned release (SFT + RLHF), and (c) at least one community DPO variant trained from the same base. Holding the base checkpoint fixed removes the confounds that block our current inference: vendor, architecture, parameter count, and pretraining data are constant by construction, so any ASR difference is attributable to post-training methodology. A monotone ordering (base > SFT > RLHF/DPO) would support the causal claim our present design cannot make (§5.1); a flat result would indicate that robustness originates in pretraining rather than alignment. Both outcomes are informative, which makes this the cheapest high-value experiment on this list.
- **Separate scale from training approach:** our 120B-vs-20B contrast is separately confounded. A within-family sweep across three or more sizes from one vendor, with post-training held constant, would isolate the scale effect that H2 predicted.
